...
function cargarFiltros(){
    ...
    // Conciliadores
    var select_conciliadores = $('select[name="conciliador"]');
    $.ajax({
        url: window.location.pathname,
        type: 'POST',
        data: {
            'action': 'searchconciliador'
        },
        dataType: 'json',
    }).done(function (data) {
...
var tblReportePorConciliador;
// Cargar datos con resultado Todos
function getDataReportePorConciliadorAll(){
    tblReportePorConciliador=$('#data').DataTable({
        responsive: true,
        lengthChange: true,
        autoWidth: false,
        destroy: true,
...
function getDataReportePorConciliadorFiltro(conciliador, materia, procedimiento, fechdesde, fechhasta, tipacta){
    tblReportePorConciliador=$('#data').DataTable({
        responsive: true,
        lengthChange: true,
        autoWidth: false,
...
$(function () { 
    ...
    getDataReportePorConciliadorAll();

    // Boton Buscar por Filtros
    $('.btnSearch').on('click', function () {

        // Capturo mis filtros
        var conciliador = $('#conciliador').val();
        ...
        if (fechdesde == '' || fechhasta == ''){
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    'action': 'searchdatosconciliador',
                    'id': conciliador
                },
                success:  function (data) {
                    // console.log(JSON.stringify(data));
                    $("#resconciliador").val(data[0] + ' - ' + data[1]);
                    if (tipacta == 'all'){
                    ...
                    getDataReportePorConciliadorFiltro(conciliador, materia, procedimiento, fechdesde, fechhasta, tipacta);                    
